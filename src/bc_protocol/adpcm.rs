...
impl AdpcmSetup {
    pub fn new_oki() -> Self {
        Self{
            ...
            mask: 15,
            step_index: 16, // Index is inti to 16 in oki
            last_output: 0, // PCM is i16, init to 0 in oki
        }
    }
}

pub fn oki_to_pcm(bytes: &[u8], oki_context: &mut AdpcmSetup) -> Vec<u8> {
    let mut result = vec![];
    ...
    for byte in bytes {
        ...
        for nibble in nibbles {
            oki_context.step_index = match oki_context.step_index as isize + oki_context.changes[*nibble as usize] { // Keep it in max index range
                n if n < 0 => 0,
            ...
            let raw_sample = oki_context.last_output as isize + diff;
            ...
            oki_context.last_output = sample;
            step = oki_context.steps[oki_context.step_index];
        }
    };
